<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="aka.MathsQuiz"%>
<%@ page import="java.util.Random" %>
<%@ page import="aka.StoreResult" %>
<!DOCTYPE html>
<html>
<head>
...
<%
    ...
    if (start.equals("Start Quiz")) {
        ...
        StoreResult.storeData((int) session.getAttribute("count"),
                (String) session.getAttribute("myq"),
                (String) session.getAttribute("mya"),
                (String) session.getAttribute("myb"),
                (String) session.getAttribute("myc"),
                (String) session.getAttribute("myd"),
                (int) session.getAttribute("usans"),
                (int) session.getAttribute("myans"));
    }
...
<%
    ...
    // Ensure maxsrno is positive before calling nextInt()
    if (maxsrno <= 0) {
%>
        <script>alert("Sorry, Not Enough Questions Yet!"); history.back();</script>
<%
    } else {
...
%>
            <script>alert("Questions Not Found!"); location.href="Maths_Exam.jsp";</script>
<%
        }
...
%>
        <script>location.href="Maths_Quiz.jsp";</script>
<%
    }
}
%>
